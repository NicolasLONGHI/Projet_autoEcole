...
public class CtrlLicence {
    ...
    public boolean VerifLicenceExiste(int idMoniteur, int idCategorie) {
        boolean licenceExiste = true;
        try {
            ps = cnx.prepareStatement("SELECT COUNT(licence.CodeLicence) FROM licence WHERE licence.CodeMoniteur = ? AND licence.CodeCategorie = ?");
            ps.setInt(1, idMoniteur);
            ps.setInt(2, idCategorie);
            rs = ps.executeQuery();
            rs.next();
